...
def calcul_amplitude():
    ...
    for i in range(len(liste_valeurs_e)):
        ...
        for k in range(360):
            O01 = k*2*pi/360
            # Mélanie #les tetha0 et tetha1 ils sont pas en fonction de alpha ??
            L.append(zero_par_secante(0, alpha*pi/180, O01, 10**-6, f1, e))
        # du coup j'ai mis ça,  ça change pas pour celui là mais la chaine deux ça change tout(on avait mit 0 et pi avant)
        maximum = max(L)
        minimum = min(L)
        debattement = maximum-minimum
        debattement = debattement*180/pi
        # Mélanie # ici je me suis dit que le debattement trouve etait en radian, pas en degre mais je suis pas sur que ça soit bon
...
CA = vecteur_CA(O01)
# nous avons besoin d'avoir les composantes de CA dans la suite
print("la valeur de l'angle alpha est:", angle_alpha(CA))
alpha = angle_alpha(CA)
# nous calculons la valeur de l'angle alpha qui va nous servir par la suite
# print ("la valeur de f1 est:", f1(O01, O03, e))

print(zero_par_secante(0, pi, pi/2, 10**-8, f1, 180))

print(calcul_amplitude())
# on cherche a trouver un debattement valant 15 degres,  avec une erreur de 0.5 degres et en deduire la valeur de e correspondante.
